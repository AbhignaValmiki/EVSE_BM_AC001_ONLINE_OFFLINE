...
boolean OcppOperation::sendReq(){
  ...
  if (timeout_active && millis() - timeout_start >= TIMEOUT_CANCEL){
    ...
    //If boot notification timed out comparing the string and re intiating boot notification again @Abhigna
  ...
  /*
  * @brief : Flag added for ethernet by G. Raja Sumant
  * 26/08/2022 to send request by ethernet.
  */

//if(wifi_connect == true || ethernet_enable == true)
