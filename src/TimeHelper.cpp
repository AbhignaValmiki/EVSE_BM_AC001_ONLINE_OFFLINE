...
/**
 * Writes the given unix epoch system time into jsonDateString.
 * 
 * jsonDateString: output buffer. Must be JSONDATE_LENGTH + 1 characters long (+1 for the 0-terminator).
 * Must be 0-initialized.
 * bufflength: number of chars in jsonDateString
 ...
 * json_document["timestamp"] = timestamp;
 */
bool getJsonDateStringFromGivenUnixTime(char* jsonDateString, int bufflength, time_t given_time){
  return getJsonDateStringFromGivenTime(jsonDateString, bufflength, given_time);
}
