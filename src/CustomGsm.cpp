...
void SetupGsm()
{
    ...
    if (DEBUG_OUT)
        Serial.println("[CustomSIM7672] Modem Info: ");
    ...
    if (DEBUG_OUT)
        SerialMon.println(F("[CustomSIM7672] Waiting for network..."));
    // if (!modem.waitForNetwork(120000))
    if (!modem.waitForNetwork(20000))
    {
        if (DEBUG_OUT)
            SerialMon.println(F("[CustomSIM7672] fail"));
        delay(200);
        return;
    }
    if (DEBUG_OUT)
        SerialMon.println(F("[CustomSIM7672] success"));

    if (modem.isNetworkConnected())
    {
        if (DEBUG_OUT)
            SerialMon.println(F("[CustomSIM7672] Network connected"));
    }
    ...
    {
        if (DEBUG_OUT)
            SerialMon.println(F("[CustomGsm]GPRS connected"));
    }
...
bool sendFrame(WSopcode_t opcode, uint8_t *payload, size_t length, bool fin, bool headerToPayload)
{
  ...
  */
    ...
    {
        // if(!headerToPayload && ((length > 0) && (length < 1600)) && (GET_FREE_HEAP > 6000)) {
