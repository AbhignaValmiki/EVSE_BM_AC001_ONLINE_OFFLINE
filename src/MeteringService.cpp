...
bool flag_trigger_metervalues_A_serve = false;
bool flag_trigger_metervalues_B_serve = false;
bool flag_trigger_metervalues_C_serve = false;
...
float current_energy_B = 0;
float current_energy_C = 0;

float discurrEnergy_A = 0;
float discurrEnergy_B = 0;
float discurrEnergy_C = 0;
extern float LastPresentEnergy_A;
extern float LastPresentEnergy_B;
extern float LastPresentEnergy_C;
...
void MeteringService::addDataPoint(time_t currentTime, float currentPower, float currentEnergy, float currentVoltage, float currentCurrent, float currentTemperature, int connector)
{
	...
	/*
	 * @brief : When trigger message is sent, it should add into the LL and then flush!
	 */

	/*
	 * @brief : This is necessary to avoid multiple flushes.
	 */
	if (flag_trigger_metervalues_A)
	{
		// Flush it!
		Serial.println("[MeteringService] Flush for A!");
	...
	{
		// Flush it!
		Serial.println("[MeteringService] Flush for B!");
		flushLinkedListValues(connector);
		return;
	}
	...
	{
		// Flush it!
		Serial.println("[MeteringService] Flush for C!");
		flushLinkedListValues(connector);
		return;
	}
	...
	/*if(flag_trigger_metervalues_A || flag_trigger_metervalues_B || flag_trigger_metervalues_C)
	  {
	   // Flush it!
	   Serial.println(F("[MeteringService] Faulted after addDataPoint!"));
   flushLinkedListValues(connector);
	  }*/
	...
	else if (connector == 1 && getChargePointStatusService_A() != NULL && getChargePointStatusService_A()->getConnectorId() == 1 && getChargePointStatusService_A()->getEvDrawsEnergy() == true)
	{
		...
		if (powerA.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && energyA.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && voltageA.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && currentA.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && temperature.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH)
		{
	...
	else if (connector == 2 && getChargePointStatusService_B() != NULL && getChargePointStatusService_B()->getConnectorId() == 2 && getChargePointStatusService_B()->getEvDrawsEnergy() == true)
	{
		 ...
		 */
		if (powerB.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && energyB.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && voltageB.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && currentB.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && temperature.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH)
		{
	...
	else if (connector == 3 && getChargePointStatusService_C() != NULL && getChargePointStatusService_C()->getConnectorId() == 3 && getChargePointStatusService_C()->getEvDrawsEnergy() == true)
	{
		 ...
		 * Check if to send all the meter values to the server
		 */
		if (powerC.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && energyC.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && voltageC.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && currentC.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH && temperature.size() >= METER_VALUES_SAMPLED_DATA_MAX_LENGTH)
		{
...
void MeteringService::flushLinkedListValues(int connector)
{
	...
	/*
	 * @brief : Through trigger!
	 */
	/*if(flag_trigger_metervalues_A || flag_trigger_metervalues_B || flag_trigger_metervalues_C)
	{
		Serial.println(F("[MeteringService] Faulted before flush!"));
		...
		Serial.println(F("[MeteringService] Faulted after flush!!"));
	}*/

	if (flag_trigger_metervalues_A)
	{
		if (flag_trigger_metervalues_A_serve)
		{
			flag_trigger_metervalues_A_serve = false;
			Serial.println(F("[MeteringService] For trigger message of A!"));
		...
		{
			Serial.println(F("[MeteringService] Avoiding duplication of A!"));
	...
	{
		if (flag_trigger_metervalues_B_serve)
		{
			flag_trigger_metervalues_B_serve = false;
			Serial.println(F("[MeteringService]For trigger message of B!"));
			if (gu8_online_flag == 1)
			{
				OcppOperation *meterValues = makeOcppOperation(webSocket,
		...
		{
			Serial.println(F("[MeteringService] Avoiding duplication of B!"));
	...
	{
		if (flag_trigger_metervalues_C_serve)
		{
			flag_trigger_metervalues_C_serve = false;
			Serial.println(F("[MeteringService] For trigger message of C!"));
			if (gu8_online_flag == 1)
			{
				OcppOperation *meterValues = makeOcppOperation(webSocket,
															   new MeterValues(connector, &sampleTimeC, &powerC, &energyC, &voltageC, &currentC, &temperature)); // have to pass connector
				initiateOcppOperation(meterValues);
			}
			sampleTimeC.clear();
			powerC.clear();
		...
		{
			Serial.println(F("[MeteringService] Avoiding duplication of C!"));
	...
	if (connector == 1)
	{
		...
		if (powerA.size() == 0 && energyA.size() == 0 && voltageA.size() == 0 && currentA.size() == 0 && temperature.size() == 0)
			return;
	...
	else if (connector == 2)
	{
		...
		if (powerB.size() == 0 && energyB.size() == 0 && voltageB.size() == 0 && currentB.size() == 0 && temperature.size() == 0)
			return;
	...
	else if (connector == 3)
	{
		...
		if (powerC.size() == 0 && energyC.size() == 0 && voltageC.size() == 0 && currentC.size() == 0 && temperature.size() == 0)
			return;
...
void MeteringService::loop()
{
	...
	/*
	 * @brief : When trigger message exists, it should addDataPoint for A, B and C.
	 */
	if (flag_trigger_metervalues_A && count_trigger >= 1)
	{
		// Add a dummy data point
		Trigger_connectorID = 1;
		count_trigger--;
		addDataPoint(0, 0, 0, 0, 0, 0, 1); // 1 = conector ID
		return;
	}

	else if (flag_trigger_metervalues_B && count_trigger >= 1)
	{
		// Add a dummy data point
		Trigger_connectorID = 2;
		count_trigger--;
		addDataPoint(0, 0, 0, 0, 0, 0, 2); // 2 = conector ID
		return;
	}

	else if (flag_trigger_metervalues_C && count_trigger >= 1)
	{
		// Add a dummy data point
		Trigger_connectorID = 3;
		count_trigger--;
		addDataPoint(0, 0, 0, 0, 0, 0, 3); // 3 = conector ID
		return;
	}

	//@bug: fix it for multiple connectors.
	if (DEBUG_OUT_M)
		Serial.println(F("The last sample time is: "));
	if (DEBUG_OUT_M)
		Serial.println(lastSampleTimeA + /*METER_VALUE_SAMPLE_INTERVAL*/ meterSampleInterval);
	if (DEBUG_OUT_M)
		Serial.println(F("The now() is: "));
	if (DEBUG_OUT_M)
		Serial.println(now());
	...
	if ((now() >= (time_t) /*METER_VALUE_SAMPLE_INTERVAL*/ meterSampleInterval + lastSampleTimeA))
	{
		if (energyASampler != NULL && powerASampler != NULL && voltageASampler != NULL && currentASampler != NULL && temperatureSampler != NULL)
		{
			...
			//removed taking metervalues in offline charging @Abhigna
			...
			if (!offline_charging_B)
			{
				...
				float sampledEnergy_B = setPresentEnergyBSampler(sampledVoltage_B, sampledCurrent_B, deltaB);
			...
			if (!offline_charging_C)
			{
				...
				float sampledEnergy_C = setPresentEnergyCSampler(sampledVoltage_C, sampledCurrent_C, deltaC);
			...
			if (DEBUG_OUT_M)
				Serial.println(F("Inside if cond."));
...
void MeteringService::init(MeteringService *meteringService)
{
	...
	if (isInitialized == 22121996)
	{
		...
		//changed the value because when power recycled metervalues are again starting from '0' when session is going on and value increased by 1000000 @Abhigna
...
float MeteringService::currentEnergy_B()
{
	...
	float finalEnergy_B = setPresentEnergyBSampler(volt_B, curr_B, delta_B);

	energymeter.putFloat("currEnergy_B", finalEnergy_B);

	if (DEBUG_OUT)
		Serial.println("The last Energy_B is: " + String(lastEnergy_B));
	if (DEBUG_OUT)
		Serial.println("The current Energy_B is: " + String(finalEnergy_B));
	// energymeter.end();
	online_charging_Enargy_B = finalEnergy_B;
	energymeter.begin("MeterData", false);
...
float MeteringService::currentEnergy_C()
{
	...
	float finalEnergy_C = setPresentEnergyCSampler(volt_C, curr_C, delta_C);

	energymeter.putFloat("currEnergy_C", finalEnergy_C);

	if (DEBUG_OUT)
		Serial.println("The last Energy_C is: " + String(lastEnergy_C));
	if (DEBUG_OUT)
		Serial.println("The current Energy_C is: " + String(finalEnergy_C));
	// energymeter.end();

	online_charging_Enargy_C = finalEnergy_C;
...
float MeteringService::currentEnergy_A()
{
	...
	float finalEnergy_A = setPresentEnergyASampler(volt_A, curr_A, delta_A);
...
float MeteringService::setPresentEnergyBSampler(float volt, float current, time_t delta)
{
	// if(delta >= 30 )
	// {
	// 	delta = 30;
	...
	energymeter.begin("MeterData", false); // opening nmespace with R/W access
	float lastEnergy_B = energymeter.getFloat("currEnergy_B", 0);
	float finalEnergy_B = lastEnergy_B + ((float)(volt * current * ((float)delta))) / 3600; // Whr

	// placing energy value back in EEPROM
	energymeter.putFloat("currEnergy_B", finalEnergy_B);
	energymeter.end();
	current_energy_B = current_energy_B + ((float)(volt * current * ((float)delta))) / 3600;
	Serial.println(" currEnergy_B: " + String(current_energy_B));
	discurrEnergy_B = (current_energy_B - LastPresentEnergy_B)/1000;
	Serial.println("discurrEnergy_B: "+String(discurrEnergy_B));
	Serial.println("current_energy_B: "+String(current_energy_B));
	Serial.println("LastPresentEnergy_B: "+String(LastPresentEnergy_B));
	Serial.println("[EnergyBSampler] finalEnergy_B: " + String(finalEnergy_B));
	return roundingFloat(finalEnergy_B);
}

float MeteringService::setPresentEnergyCSampler(float volt, float current, time_t delta)
{
	// if(delta >= 30 )
	// {
	// 	delta = 30;
	...
	energymeter.begin("MeterData", false); // opening nmespace with R/W access
	float lastEnergy_C = energymeter.getFloat("currEnergy_C", 0);
	float finalEnergy_C = lastEnergy_C + ((float)(volt * current * ((float)delta))) / 3600; // Whr

	// placing energy value back in EEPROM
	energymeter.putFloat("currEnergy_C", finalEnergy_C);
	energymeter.end();
	current_energy_C = current_energy_C + ((float)(volt * current * ((float)delta))) / 3600;
	discurrEnergy_C = (current_energy_C - LastPresentEnergy_C)/1000;
	Serial.println("discurrEnergy_C: "+String(discurrEnergy_C));
	Serial.println(" currEnergy_C: " + String(current_energy_C));
	Serial.println("LastPresentEnergy_C: "+String(LastPresentEnergy_C));
	Serial.println("[EnergyCSampler] finalEnergy_C: " + String(finalEnergy_C));
