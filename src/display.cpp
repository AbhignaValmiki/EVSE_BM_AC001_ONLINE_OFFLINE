...
void connAvail(uint8_t con, String state)
{
  ...
  {
  case 1:
    data_tx["connector1"] = state; //(char *)
    break;
  case 2:
    data_tx["connector2"] = state; //(char *)
    break;
  case 3:
    data_tx["connector3"] = state; //(char *)
    break;
  }
  serializeJson(data_tx, Serial);
  serializeJson(data_tx, display);
...
bool checkForResponse_Disp()
{
  ...
  while (millis() - startTime < WAIT_TIMEOUT_DISP)
  { // waiting for response from slave
    if (display.available())
    {
      ReadLoggingStream loggingStream(display, Serial);
...
void startmastertxn()
{
  ...
  while (flag_stopOnlineCommunication == true)
  {
    ...
    if (EVSE_state == STATE_C || EVSE_state == STATE_D)
    {
      ...
      if (DEBUG_OUT)
        Serial.println("[EVSE_M_P] Drawing Energy");
    }
    ...
    else if (EVSE_state == STATE_DIS || EVSE_state == STATE_E)
    { // thank you
      flag_chargingM = false;
